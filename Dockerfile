...
RUN dnf5 up -y
RUN dnf5 install -y nodejs docker xorg-x11-server-Xvfb lorax wget ostree ostree-devel rpm-ostree git hfsplus-tools flatpak flatpak-builder dbus dbus-daemon rpmdevtools make mock selinux-policy python3-pip jq @development-tools gcc cairo-devel python3-devel python3-aiohttp python3-tenacity python3-gobject 'pkgconfig(gobject-introspection-1.0)' 'pkgconfig(cairo-gobject)' 'pkgconfig(gnome-desktop-4)' clang-devel meson
...
RUN dnf5 -y install melody vala-lint-nightly libhelium-devel sass
